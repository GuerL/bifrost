...
pub fn environments_dir(app: &AppHandle) -> Result<PathBuf, String> {
    Ok(app_dir(app)?.join("environments"))
}

pub fn environment_path(app: &AppHandle, environment_id: &str) -> Result<PathBuf, String> {
    Ok(environments_dir(app)?.join(format!("{environment_id}.json")))
}

pub fn environments_index_path(app: &AppHandle) -> Result<PathBuf, String> {
    Ok(environments_dir(app)?.join("index.json"))
}
